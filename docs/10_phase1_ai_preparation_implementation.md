...
- ステータス: **履歴。3入力・30試行（旧clap）時点の先行実装記録。現行の試験条件（定例は残る2動作・20試行、P1結果schema v7）と現在地は[資料ガイド](./README.md)と[POCテスト手順](./05_poc_test_protocol.md)の5章を見る**
- 文書種別: 実装結果と引き継ぎ。仕様の正本ではない
- 注記: 本文の「30試行」「クラップ」「schema version 2／3」「実機確認JSON schema 2.1」は2026-07時点の値である。現行は定例が残る2動作・20試行（5動作・50試行は`?protocol=five`）、Bloom、P1結果schema v7、診断リプレイschema v3、実機確認JSON schema 2.4（[13](./13_p1_five_gesture_50_trial_revision_plan.md)の14章）
