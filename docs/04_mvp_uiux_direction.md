...
スマートフォン横向きのPhase 1では、最初の画面をカメラとP1-Controlled操作の二列へ固定する。カメラは4:3の取得画角を保ち、横長の枠へ`cover`して上下を切り落とさない。常時見せるのは追跡状態、開始／停止、試行進捗、現在の指示、手アイコンの動作見本、試行状態、残り時間、直近の拒否理由、手動分類、「未成立として次へ」、JSON保存だけとする。現行P1の3入力はair-tap、ribbon-swipe、Bloomであり、旧clapは履歴JSONの読込専用とする。

縦向きで開いた場合は、ユーザー操作を起点に全画面化と`Screen Orientation API`の横向き固定を試す。固定APIを利用できないブラウザでは、端末の自動回転を有効にして物理的に横向きへ置く案内を表示する。カメラ開始後の「テストを開始」は、音声クロック、新しいP1セッション、1試行目の準備を一続きで開始し、セッション作成後に別の開始操作を要求しない。

試行状態は`準備中`、`GO`、`判定中`、`未成立を記録`、`完了`を短く表示する。残り時間はdeadlineとの差を秒単位で示し、更新回数を時刻の根拠にしない。拒否理由は同じ文言を毎フレーム点滅させず、直近の理由と発生件数を日本語1行で保持する。主な表示は次とする。
...
| `bloom-not-outward` / `bloom-not-upward` | Bloomの手の向きが外側／上向きから外れました |
| `bloom-sync-expired` | Bloomの左右の到達が同期幅を超えました |
...
- controlled基準候補: 3入力が8/10以上でも、対象端末／テスターと主観評価の手動確認が必要だと表示する。旧clapとBloomが混在する場合は候補にしない。
- 自動`Pass`: 表示しない。
