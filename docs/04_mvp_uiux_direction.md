...
スマートフォン横向きのPhase 1では、最初の画面をカメラとP1-Controlled操作の二列へ固定する。カメラは4:3の取得画角を保ち、横長の枠へ`cover`して上下を切り落とさない。常時見せるのは追跡状態、開始／停止、試行進捗、現在の指示、手アイコンの動作見本、試行状態、残り時間、直近の拒否理由、手動分類、「未成立として次へ」、JSON保存だけとする。
...
- 実接触クラップ: 離した両手を中央へ寄せ、手のひらをそっと合わせる。
- ニアクラップ: 離した両手を中央へ寄せ、光球を挟んで触れずに止める。
...
実機確認レポートの後に、PC向けの「P1セッション比較」を置く。複数のP1 JSONを読み込み、セッション、build、profile、30試行の整合、3ジェスチャーのsuccess、tracking Hz、frame age p95、二手coverage、警告を横並びで確認できるようにする。比較結果は次の表現を使う。

- 不完全または条件混在: 何が不足・相違しているかを具体的に表示する。
- 負荷変更候補: tracking Hzまたはframe ageから、次セッションで処理負荷を一項目比較する候補を示す。
- controlled基準候補: 3ジェスチャーが8/10以上でも、対象端末／テスターと主観評価の手動確認が必要だと表示する。
- 自動`Pass`: 表示しない。
