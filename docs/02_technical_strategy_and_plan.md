...
- ステータス: v0.4（P1を5動作・50試行へ拡張。Bloomの準備完了後の合図、Lift／Spotlight候補、schema v5を追加。2026-09-19: セッション比較の記述をschema v5以降へ直した。記録項目の正本は05の5.7。9.2へ取得経路のURL指定を追記。7.3の追跡の途切れの許容をフレーム間隔に合わせた）
...
| 論点 | 当初案 | 再評価後 | 変更理由 |
|---|---|---|---|
| 追跡対象 | 片手を標準、必要時に二手 | 二手を標準、片手を明示モード | Bloom、ハイタッチ、両手ポータル、小動作から大動作への身体的フレージングを順に検証する |
...
- 現行Bloomが日常動作や顔の前の手振りで誤発火しないこと。旧clapの誤爆は互換語彙の将来研究として別に扱う。
...
- `handPolicy`、`reachScale`、`seated`を譜面とランタイムの正式な概念にする。旧clap互換の無音扱いは履歴／将来研究に限定し、現行Bloomは実接触・マイクを要求しない。
...
| 自前の時系列状態機械 | 採用 | タップ、スワイプ、Bloom、旧clap、開閉、レールを説明可能に判定 |
...
- 短い遮蔽: ジェスチャー別のgrace期間を置く。2026-09-19から、値は「150msと、直近のフレーム間隔の中央値の2.5倍の、大きいほう」（上限400ms）とし、trackの保持にも同じ値を使う。150ms固定では、毎秒約10回の端末でフレーム間隔が150msを超えただけでtrackIdが変わり、候補が消えていた（[19](./19_android_third_test_and_recognition_fixes.md)の3.1）。
...
同じ動きがスワイプとタップ、旧clapとツインヒットの両方に見えることがある。譜面コンテキストだけで強制成功させず、`GestureArbiter`で候補を比較する。現行P1／MVPの第三入力Bloomは、旧clapとは別の候補として扱う。
...
旧clap状態機械は、過去セッションの再生・比較と履歴の読み込みのために残す。現行P1／MVPの第三入力ではなく、Bloomと同じ条件へ集計しない。

過去の接触／ニアクラップでは、両手中心距離、収束速度、短い遮蔽の推定を使っていた。この実装と診断フィールドは互換読込用に維持するが、実接触を要求する現行動作へ戻さない。

旧実装でもマイクの短いトランジェントは任意の補助に留め、音量は評価しない。

### 8.4 Bloomの判定

現行P1のBloomは、接触やマイクを使わず、二手の相対的な外向き・上向き軌跡を独立した状態機械で判定する。

1. 二手が中央寄りの準備姿勢に入った時刻を記録する。
2. 準備中の動きは認識windowへ持ち越さず、window開始時に候補をリセットする。
3. 左手は左外側、右手は右外側へ、両手は上向きへ進んだ距離を測る。
4. 左右の閾値到達時刻を`captureTime`間で補間し、同期幅内なら成立させる。
5. 片手が先に到達したまま長く待つ場合は`bloom-sync-expired`として拒否する。
6. 短い欠落（7.3の許容時間以内）は候補を保持し、長い欠落は`tracking-lost`を一度だけ返して再アームする。
...
2026-09-20に、処理速度の切り分けを自動の速度チェックへ替えた（[20](./20_android_fourth_test_and_portrait_direction.md)の4.4）。`?mode=speedcheck`で開き、「はじめる」を1回押して両手を映したまま約2分待つと、次の7設定を順に測り、結果を1つのJSON（`oto-motion-speed-check`、schemaVersion 1）へ保存する。各設定は準備3秒を捨てて12秒を集計し、設定ごとにWorkerを作り直す。(1) GPU・640×480・取り込み自動・待機枠あり（現在の既定）、(2) 同・待機枠なし、(3) GPU・`requestVideoFrameCallback`、(4) CPU・取り込み自動、(5) CPU・`requestVideoFrameCallback`、(6) GPU・320×240、(7) 1番の再測定（発熱の影響を見分ける）。両手が映っていた割合が0.6未満の設定には`handsVisibleEnough: false`を付ける。手が映っていないと認識が軽くなり、速く見えるためである。通常の画面の既定と、`?frameSource=`／`?pending=`の挙動は変えていない。URLの指定を1つずつ変えて試行を繰り返す方式は、テスターの手間が大きいのでやめた。
...
- 結果JSONへ、profile ID、app build ID、要求解像度／fps／delegate／modelと、ブラウザが実際に選んだ解像度／fps／delegate／modelを分けて保存する。
- app build IDは明示した`VITE_APP_BUILD_ID`、CIのcommit ID、ビルド入力の内容hashの順で決定し、package versionだけで異なる実装を同一build扱いにしない。
- GPU初期化失敗によるCPU fallbackは、CPUプロファイルを選んだ場合と区別して記録する。

実測前に候補の優劣を決めない。追跡15Hz未満またはframe age p95 140ms超が続いた場合も、同じセッション内で設定を変えず、次セッションで比較するプロファイルを一つ選ぶ。
...
- 二手カーソルと21点表示
- エアタップ、リボンスワイプ、Bloom（両手の中央準備から左右斜め上への開放）
...
- ターゲットテスターが、Bloomまたはリボンスワイプを印象的な瞬間として自発的に挙げる。
...
| Bloomで二手が非同期・追跡欠落 | 片手だけ先に開く、短い欠落、長い欠落 | 外向き／上向き閾値、時刻補間、同期幅、短いgrace、長い欠落はtracking-lost |
...
8. Bloomの中央準備・両手開放状態機械（旧clap予測は互換読込用に維持）
...
この順序なら、華やかな演出を作る前に「二手が取れるか」「時刻が信頼できるか」「Bloomが同期して開くか」という最大の不確実性を減らせる。
