...
画面下部の「実機確認レポート」では、試験条件、カメラ経路、二手追跡、停止・復帰を「未確認／問題なし／問題あり／対象外」で管理できる。P1セッションJSONから3ジェスチャー結果を取り込み、実機確認JSONとして保存・読み戻しできる。旧schema 1.0のJSONも読み込み時に2.0へ移行する。JSONにカメラ映像や音声は含めない。
